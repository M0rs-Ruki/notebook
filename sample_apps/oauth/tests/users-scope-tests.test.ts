/**
 * Users scope tests. Mentor ask: user read, invite, write, delete — test separately, use asserts.
 */
...
describe('Users scope tests (user read, invite, write, delete)', () => {
  ...
  it.each(USERS_TEST_CASES)('$requiredScope — $name: with scope returns 2xx or 4xx, without scope returns 401', async (testCase) => {
    if (!accessToken) return
